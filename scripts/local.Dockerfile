...
RUN mkdir -p /go/src/github.com/SavaLabs

WORKDIR $GOPATH/src/github.com/SavaLabs
COPY savannahnode savannahnode
COPY coreth coreth

WORKDIR $GOPATH/src/github.com/SavaLabs/savannahnode
RUN ./scripts/build_savannahnode.sh
RUN ./scripts/build_coreth.sh ../coreth $PWD/build/plugins/evm

RUN ln -sv $GOPATH/src/github.com/SavaLabs/savannahnode-byzantine/ /savannahnode
