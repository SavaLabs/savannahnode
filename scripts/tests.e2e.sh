...
SAVANODE_PATH="${1-}"
if [[ -z "${SAVANODE_PATH}" ]]; then
  echo "Missing SAVANODE_PATH argument!"
  echo "Usage: ${0} [SAVANODE_PATH]" >> /dev/stderr
  exit 255
fi
...
echo "running e2e tests against the local cluster with ${SAVANODE_PATH}"
./tests/e2e/e2e.test \
--ginkgo.v \
--log-level debug \
--network-runner-grpc-endpoint="0.0.0.0:12342" \
--network-runner-avalanchego-path=${SAVANODE_PATH} \
