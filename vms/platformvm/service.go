...
import (
	...
	"github.com/SavaLabs/savannahnode/vms/components/keystore"
	...
	"github.com/SavaLabs/savannahnode/vms/platformvm/txs/builder"
	"github.com/SavaLabs/savannahnode/vms/platformvm/txs/executor"
	"github.com/SavaLabs/savannahnode/vms/secp256k1fx"

	platformapi "github.com/SavaLabs/savannahnode/vms/platformvm/api"
)
...
// Note: We explicitly duplicate FUEL out of the maps to ensure backwards
// compatibility.
type GetBalanceResponse struct {
	// Balance, in nFUEL, of the address
...
// GetCurrentSupply returns an upper bound on the supply of FUEL in the system
...
// ImportAVAX issues a transaction to import FUEL from the X-chain. The FUEL
// must have already been exported from the X-Chain.
func (service *Service) ImportAVAX(_ *http.Request, args *ImportAVAXArgs, response *api.JSONTxIDChangeAddr) error {
	service.vm.ctx.Log.Debug("Platform: ImportFUEL called")
...
// GetStake returns the amount of nFUEL that [args.Addresses] have cumulatively
// staked on the Primary Network.
//
// This method assumes that each stake output has only owner
// This method assumes only FUEL can be staked
...
// GetMinStake returns the minimum staking amount in nFUEL.
...
// GetMaxStakeAmount returns the maximum amount of nFUEL staking to the named
